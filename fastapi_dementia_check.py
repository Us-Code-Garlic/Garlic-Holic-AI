    ...
    try:
        ...
        if not baseline_embeddings:
            ...
            for baseline_embedding_blob in baseline_embeddings:
                ...
                similarities.append(float(similarity))  # numpy.float32를 float로 변환
            
            similarity_score = float(np.mean(similarities))  # numpy.float32를 float로 변환
...
def check_repetitive_speech(state: DementiaCheckState):
    ...
    try:
        ...
        if not previous_conversations:
            ...
            repetition_score = float(max_similarity)  # numpy.float32를 float로 변환
...
def combine_diagnosis(state: DementiaCheckState):
    ...
    total_confidence = 0.0  # float 타입으로 명시
    details = []
